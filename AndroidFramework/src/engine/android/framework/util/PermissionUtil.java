...
public class PermissionUtil {
    ...
    public void requestPermission(String... permissions) {
        if (checkPermission(activity, permissions))
        {
            if (callback != null)
            {
                callback.onRequestPermissionsResult(0, permissions, new int[0]);
            }
        ...
        activity.requestPermissions(list.toArray(new String[list.size()]), 0);
    }
    ...
    private static List<String> getNeedPermission(Activity activity, String[] permissions) {
        List<String> needPermissionList = new ArrayList<String>();
        for (String permission : permissions)
        {
            if (activity.checkSelfPermission(permission) != PackageManager.PERMISSION_GRANTED
