package com.daan.project.ui.widget;
...
public class XYChartView extends View {
    ...
    /***** 各边距 *****/
    private final int spaceLeft     = 25;
    private final int spcaeTop      = 10;
    private final int spaceRight    = 10;
    private final int spaceBottom   = 25;
    ...
    private int paintColor = paint.getColor();      // 画笔颜色
    private float textSize = paint.getTextSize();   // 字体大小
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int spaceLeft = (int) TypedValue.applyDimension(unit, this.spaceLeft, metrics);
        int spaceRight = (int) TypedValue.applyDimension(unit, this.spaceRight, metrics);
        int spcaeTop = (int) TypedValue.applyDimension(unit, this.spcaeTop, metrics);
        int spaceBottom = (int) TypedValue.applyDimension(unit, this.spaceBottom, metrics);
        
        if ((xLength = w - spaceLeft - spaceRight) <= 0 
        ||  (yLength = h - spcaeTop - spaceBottom) <= 0)
        {
            return;
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        yMax = FloatMath.ceil(yMax);
        {
            ...
            pow /= 10;
            int len = (int) Math.ceil(segmentX * xMax / xLength);
        ...
        
        {
            ...
            pow /= 10;
            int len = (int) Math.ceil(segmentY * yMax / yLength);
            while (len <= 0 || len % pow != 0)
            {
                len++;
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(xUnit))
        {
            ...
            canvas.drawText(xUnit, x - 15, y + textSize, paint);
        }
        ...
        {
            paint.setTextSize(textSize);
            canvas.drawText(yUnit, x + 12, y + 15, paint);
        }
    ...
    private void drawAxis(Canvas canvas, int segmentX, int segmentY) {
        ...
        for (int i = 0; i < xNum; i++)
        {
            ...
            if (coordsX != null) canvas.drawText(coordsX[i], x, y + 5 + textSize, paint);
            x += segmentX;
        }
        ...
        for (int i = 0; i < yNum; i++)
        {
            ...
            if (coordsY != null) canvas.drawText(coordsY[i], x - 10, y + 5, paint);
            y -= segmentY;
        } 
    }
    ...
    private void drawLine(Canvas canvas, int segmentX, XYChartLine line, int lineX) {
        ...
        if (!TextUtils.isEmpty(line.name))
        {
