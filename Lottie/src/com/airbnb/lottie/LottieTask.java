...
public class LottieTask<T> {
  ...
  LottieTask(Callable<LottieResult<T>> runnable, boolean runNow) {
    task = new FutureTask<LottieResult<T>>(runnable);
  ...
  private void setResult(LottieResult<T> result) {
    if (this.result != null) {
  ...
  private void notifySuccessListeners(T value) {
    ...
    List<LottieListener<T>> listenersCopy = new ArrayList<LottieListener<T>>(successListeners);
  ...
  private void notifyFailureListeners(Throwable e) {
    // Allows listeners to remove themselves in onResult.
    // Otherwise we risk ConcurrentModificationException.
    List<LottieListener<Throwable>> listenersCopy = new ArrayList<LottieListener<Throwable>>(failureListeners);
